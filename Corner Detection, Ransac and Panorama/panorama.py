...
def harris_corners(img, window_size=3, k=0.04):
    ...
    dy2=dy**2
    dxdy=dx*dy
    my = convolve(dy2, window, mode='constant')
    mx = convolve(dx2, window, mode='constant')
    mxy = convolve(dxdy, window, mode='constant')
    response = (mx * my - mxy ** 2) - k * (mx + my) ** 2
    ### END YOUR CODE
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for _ in range(n_iters):
        ...
        H[:,2] = np.array([0, 0, 1])
        curr_in = np.sqrt(((matched2.dot(H) - matched1) ** 2).sum(axis=-1)) < threshold
        curr_n = curr_in.sum()
        if curr_n > n_inliers:
            max_inliers, n_inliers = curr_in, curr_n
...
def hog_descriptor(patch, pixels_per_cell=(8,8)):
    ...
    ### YOUR CODE HERE
    low_bins = ((theta_cells // degrees_per_bin - (theta_cells % degrees_per_bin <= degrees_per_bin / 2)) % n_bins).astype(int)
    high_bins = ((low_bins + 1) % n_bins).astype(int)
    low_weights = ((degrees_per_bin / 2 - theta_cells % degrees_per_bin) % degrees_per_bin) / degrees_per_bin
    high_weights = 1 - low_weights
    indices = np.indices((rows, cols, pixels_per_cell[0], pixels_per_cell[1]))
    i, j = indices[0].reshape(-1), indices[1].reshape(-1)
    np.add.at(cells, (i, j, low_bins.reshape(-1)), (low_weights * G_cells).reshape(-1))
    np.add.at(cells, (i, j, high_bins.reshape(-1)), (high_weights * G_cells).reshape(-1))
...
def linear_blend(img1_warped, img2_warped):
    ...
    if left_margin < right_margin:
        ...
        weight_1[:, left_margin:right_margin + 1] = np.linspace(1, 0, n)
        weight_2[:, left_margin:right_margin + 1] = np.linspace(0, 1, n)
    merged = weight_1 * img1_warped + weight_2 * img2_warped   
